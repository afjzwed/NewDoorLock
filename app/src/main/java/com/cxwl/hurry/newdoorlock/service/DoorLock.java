 ...
 * 锁相开源门禁机软件的主要服务类,newdoorlock主要提供开门,关门指令以及上报门开和关闭的事件.
 */
public class DoorLock extends Service implements OnBackCall {

    public static final String TAG = "newdoorlock";
    public static final String mDoorSensorAction = "com.android.action.doorsensor";

    private newdoorlockServiceBinder mnewdoorlock;

    /**
     * 当门的状态改变时的事件定义
     */
    public static final String newdoorlockStatusChange = "newdoorlockStatusChange";
    /**
     * newdoorlock通过newdoorlockOpenDoor广播获得开门指令并发送给门禁控制器
     */
    public static final String newdoorlockOpenDoor = "newdoorlockOpenDoor";
    public static final String newdoorlockOpenDoor_BLE = "newdoorlockOpenDoor_BLE";
    ...
    public void onCreate() {
        super.onCreate();
        mnewdoorlock = new newdoorlockServiceBinder();
        mReceiver = new NotifyReceiver();
        IntentFilter filter = new IntentFilter();
        filter.addAction(mDoorSensorAction);
        filter.addAction(newdoorlockOpenDoor);
    ...
    public class newdoorlockServiceBinder extends IDoorLockInterface.Stub {
    ...
    public class NotifyReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(mDoorSensorAction)) {
                ...
                ds_intent.setAction(DoorLock.newdoorlockStatusChange);
                ds_intent.putExtra("doorsensor", mds.get("doorsensor"));
                sendBroadcast(ds_intent);
            } else if (intent.getAction().equals(newdoorlockOpenDoor)) {
                int index = intent.getIntExtra("index", 0);
                int status = intent.getIntExtra("status", 0);
                if (status != 0) {
                    mnewdoorlock.openDoor(0xF0, 0x40);
                    //mnewdoorlock.openDoor(1, 0x20);
                } else {
                    int result = mnewdoorlock.closeDoor(index);
                }
                    ...
                    try {
                        switch ((int) dp.property_id) {
                            case 1600006:   //主门开锁
                            {
                                ...
                                Intent ds_intent = new Intent();
                                ds_intent.setAction(newdoorlock.newdoorlockOpenDoor);
                                ...
                                intent_ble.setAction(newdoorlockOpenDoor_BLE);
